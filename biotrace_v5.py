...
# [ENHANCEMENT: biotrace_dedup_patch]
try:
    from biotrace_dedup_patch import dedup_occurrences, suppress_regional_duplicates
except ImportError:
    def dedup_occurrences(occs): return occs, 0
    def suppress_regional_duplicates(occs): return occs, 0
...
verify_occurrence_names = None
find_species_with_gnfinder = None   # ← GNA Finder API mapped properly
try:
    from species_verifier import verify_occurrence_names, find_names_in_text as find_species_with_gnfinder

    _VERIFIER_AVAILABLE = True
    logger.info("[v5] species_verifier loaded (verify + GNA Finder)")
except ImportError:
    logger.warning("[v5] species_verifier.py not found or missing exports")
...
dedup_occurrences   = None

try:
    from biotrace_gnv import (
        GNVEnrichedVerifier, LocalitySplitter,
        safe_parse_json, dedup_occurrences,
    )
    _GNV_AVAILABLE = True
    logger.info("[v5] GNVEnrichedVerifier + LocalitySplitter loaded")
except ImportError:
...
def extract_thinker(
    ...
    if find_species_with_gnfinder  is not None:
        try:
            ...
            logger.debug("[thinker/GNA] %s", exc)

    # ── Layer 2: BiodiViz HF NER ──────────────────────────────────────────────
    if _BIODIVIZ_AVAILABLE:
        try:
            hf_pipeline = get_biodiviz_pipeline()
            if hf_pipeline:
                hf_results = hf_pipeline.extract(chunk_text[:6000])
                hf_names   = hf_results.get("organisms", [])
                if hf_names:
                    found.extend(hf_names)
                    log_cb(f"    [BiodiViz NER] {len(hf_names)} organisms detected")
        except Exception as exc:
            logger.debug("[thinker/HF] %s", exc)
...
def enrich_taxonomy(occurrences: list[dict], log_cb, wiki=None) -> list[dict]:
    """v5.1: GNV-enriched verifier with full field mapping and wiki profile update."""
    if not occurrences:
        return occurrences

    # Prefer GNVEnrichedVerifier (v5.1) over base species_verifier
    if _GNV_AVAILABLE and GNVEnrichedVerifier:
        log_cb(f"[Taxonomy] GNV verifying {len(occurrences)} records (WoRMS + CoL + GBIF)…")
        try:
            verifier = GNVEnrichedVerifier(wiki=wiki)
            return verifier.verify_and_enrich(occurrences, update_wiki=(wiki is not None))
        except Exception as exc:
            log_cb(f"[Taxonomy] GNV error: {exc} — falling back to base verifier", "warn")

    if verify_occurrence_names is None:
        log_cb("[Taxonomy] species_verifier unavailable — skipped", "warn")
        return occurrences
    log_cb(f"[Taxonomy] Base verifier: {len(occurrences)} records…")
    try:
        return verify_occurrence_names(occurrences)
    except Exception as exc:
...
with st.sidebar:
    ...
    _status(_VERIFIER_AVAILABLE,   "species_verifier.py",          "species_verifier.py",          is_local=True)

...
def find_names_in_text(text: str) -> list[dict]:
    """
    Returns list of {scientificName, verbatimName, cardinality} for binomials+.
    """
    ...
    try:
        r = requests.post(GNA_FINDER_URL,
                          ...
                          timeout=_TIMEOUT)
        r.raise_for_status()
        return [{"scientificName": n.get("name",""),
                 "verbatimName":   n.get("verbatim", n.get("name","")),
                 "cardinality":    n.get("cardinality",2)}
                for n in r.json().get("names",[]) if n.get("cardinality",0) >= 2]
